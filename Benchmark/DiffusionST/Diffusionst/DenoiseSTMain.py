import torch
from Diffusionst.preprocess import preprocess_adj, preprocess_adj_sparse, preprocess, construct_interaction, construct_interaction_KNN, add_contrastive_label, get_feature, permutation, fix_seed
import time
import random
import numpy as np
from Diffusionst.model import Encoder, Encoder_sparse, Encoder_map, Encoder_sc, ZINBLoss, GaussianNoise, MeanAct, DispAct
...
class DenoiseST():
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            if nan_count.item()>0:
                self.emb=copyemb
                # print("1111111111111111",self.emb)
                break
        ...
        with torch.no_grad():
            ...
            if self.deconvolution:
                ...
                # print("222222222",self.adata.obsm['emb'])
                # print("33333333",self.emb)
                # print("4444444",self.emb_rec)
                # print("5555555",self.emb.detach().cpu().numpy())
                return self.adata
