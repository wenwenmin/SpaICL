...
def detect_spatial_domains_ez_mode(adata, img, x_array, y_array, x_pixel, y_pixel, n_clusters, histology=True, s=1, b=49, p=0.5, r_seed=100, t_seed=100, n_seed=100, init="louvain"):
	...
	sc.pp.normalize_total(adata) # old code: sc.pp.normalize_per_cell(adata)
	sc.pp.log1p(adata)
	...
	clf.train(adata, adj, init_spa=True, init=init, res=res, tol=5e-3, lr=0.05, max_epochs=200, n_clusters=n_clusters)
