...
def calculate_adj_matrix(x, y, x_pixel=None, y_pixel=None, image=None, beta=49, alpha=1, histology=True):
	...
	if histology:
		...
		for i in range(len(x_pixel)):
			...
			max_y=image.shape[1]
			# old version
			# nbs=image[round(max(0,x_pixel[i]-beta_half)):round(min(max_x,x_pixel[i]+beta_half+1)),round(max(0,y_pixel[i]-beta_half)):round(min(max_y,y_pixel[i]+beta_half+1))]
			x_s = round(max(0, x_pixel[i] - beta_half))
			x_e = round(min(max_y, x_pixel[i] + beta_half))
			y_s = round(max(0, y_pixel[i] - beta_half))
			y_e = round(min(max_x, y_pixel[i] + beta_half))
			nbs = image[y_s:y_e, x_s:x_e]
			g.append(np.mean(np.mean(nbs,axis=0),axis=0))
		c0, c1, c2=[], [], []
		for i in g:
			c0.append(i[0])
